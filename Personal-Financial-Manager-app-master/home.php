...
<!-- login form -->
<div class="mx-2 mb-5 p-2 mt-2 bg-light">
<h4 class="text-navy"><i class="fa fa-user-tie text-danger"></i> Welcome,

<?php
  echo $_SESSION['fname']." ".$_SESSION['sname'].' to Personal Financial Manager app';
?>
...
<div id="notification">
    <button class="btn btn-success" id="notification-bell">
...
<div id="notification-div">
    <?php
    $sql3="SELECT * FROM debt WHERE `userId`='$id' AND repay_date >= DATE_SUB(CURDATE(), INTERVAL 1 DAY)
  AND repay_date <= CURDATE() AND remain > 0";
      $qry3=mysqli_query($conn,$sql3);
      $deb=mysqli_fetch_array($qry3);
      if(mysqli_num_rows($qry3) > 0){?>
          <a href="debts.php">You need to repay <?php echo 'Ksh. '.$deb['remain']?> borrowed for <?php echo $deb['reason']?>. Repay date is <?php echo $deb['repay_date']?>.</a>
        <?php
      }
      else{?>
        <p>No Notification</p>
...
$(document).ready(function(){
    if($('#notification-div').find('a').length > 0){
        $('#notification-bell span').show();
    }
    
    $('#notification-bell').on('click', function(){
        if ($('#notification-div').css('display') === 'flex') {
            $('#notification-div').css('display', 'none');
        } else {
            $('#notification-div').css('display', 'flex');
        }
    })
})
  $(function () {
